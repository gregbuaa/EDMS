...
function load_home_page() {
	...
	$.ajax({
		...
		success: function(result, status, xhr) {
			...
			// 解析JSON数据
			result_home_page.ccn_experts = JSON.parse(result.ccn_experts);
			result_home_page.hotest_five_organizations = [];
			result_home_page.hotest_five_experts = [];

			let hotest_five_organizations_temp = JSON.parse(result.hotest_five_organizations);
			hotest_five_organizations_temp.forEach(function (item, index) {
				result_home_page.hotest_five_organizations.push(JSON.parse(item));
            });

			let hotest_five_experts_temp = JSON.parse(result.hotest_five_experts);
			hotest_five_experts_temp.forEach(function (item, index) {
				result_home_page.hotest_five_experts.push(JSON.parse(item));
            });

			console.log(result_home_page);
			console.log(result_home_page.hotest_five_organizations);
			console.log(result_home_page.hotest_five_experts);

			// 使用数据渲染页面
			// result = result_home_page;
